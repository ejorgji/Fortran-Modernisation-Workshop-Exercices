...
!  real (kind=dp) :: x(x_num)
  ...
  character (len=100) :: output_png_filename
  character(len=100) :: format_string
  character(len=100) :: png_idx
...
! remember Fortran is column major, meaning that rows are contiguous
  ...
  allocate( t(1:t_num), stat = ierr_t, errmsg = msg_t ) 
  if (ierr_t /= 0) then
    print *, msg_t
  endif
...
! the main time integration loop 
  ...
  do j = 2, t_num
	...
	if ( (mod(j,10)) == 0 ) then
	  write (png_idx, format_string) (j/10)
	  output_png_filename = 'image_'//trim(png_idx)//'.png'
	  
	  call plsdev( 'pngcairo' ) 
	  call plsfnam( output_png_filename ) 
	  call plinit( ) 
	  call plenv( 0.0, 1.0, 60.0, 100.0, 0, 0 ) 
	  call pllab( 'x', 'heat', 'heat solution Celsius' ) 
	  call plline( x(:), h_new(:) ) 
	  call plend( ) 
